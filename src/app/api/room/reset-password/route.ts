...
export async function POST(req: Request) {
  ...
  if (newPassword.length < 6) return NextResponse.json({ error: "Nýtt lykilorð þarf að vera amk 6 stafir" }, { status: 400 });
  ...
  if (mErr || !member) return NextResponse.json({ error: "Notandanafn fannst ekki" }, { status: 404 });
  ...
  return NextResponse.json({ ok: true, message: "Lykilorð hefur verið endursett" });
}
